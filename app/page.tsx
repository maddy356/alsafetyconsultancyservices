  ...
  return (
    <div className="min-h-screen bg-white text-slate-900 font-sans selection:bg-blue-100">
      ...
      <section className="py-24 px-6 text-center bg-slate-50">
        <h1 className="text-4xl md:text-6xl font-black text-slate-900 mb-6">Expert Safety <br/><span className="text-blue-600">Solutions.</span></h1>
        <p className="text-slate-500 max-w-xl mx-auto text-lg leading-relaxed italic">Consultancy services for ISO standards and workplace safety excellence.</p>
      </section>
          ...
          <div key={i} className="p-8 border border-slate-100 rounded-2xl hover:border-blue-200 hover:shadow-xl transition-all group">
            ...
            <ul className="space-y-2">
              {s.items.map((item, idx) => (
                <li key={idx} className="text-sm text-slate-500 flex items-center gap-2"><div className="w-1 h-1 rounded-full bg-blue-400" /> {item}</li>
              ))}
            </ul>
          </div>
        ))}
      ...
      <section id="contact" className="py-24 px-6 bg-slate-100">
        <div className="max-w-5xl mx-auto bg-white rounded-[2rem] shadow-xl overflow-hidden flex flex-col md:row items-stretch">
            ...
            {isSuccess ? (
              ...
              <form action={handleSubmit} className="space-y-5">
                ...
                <div className="grid md:grid-cols-3 gap-4">
                  ...
                  <input name="email" type="email" placeholder="Business Email" required className="p-4 bg-slate-50 rounded-xl outline-none focus:ring-2 focus:ring-blue-600 transition-all border border-transparent" />

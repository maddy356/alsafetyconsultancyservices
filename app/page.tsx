...
export default function LandingPage() {
  ...
  const content = {
    en: {
      ...
      vision: "To be the most trusted and recognized trademark in safety, health, environment and sustainable development consultancy – helping shape a world where every workplace is inherently safe, green and resilient.",
      ...
      services: [
        ...
        { title: "4. Risk Management & Assessments", items: ["Workplace risk and Job Safety Analysis (JSA)", "Site safety inspections", "Specialized assessments"] },
        ...
        { title: "11. Digital HSE Solutions", items: ["Digitization of SMS", "Custom real-time dashboards", "Digital audit tools"] },
    ...
    ar: {
      ...
      whoWeAreDesc: "نحن فريق من استشاريي الصحة والسلامة والبيئة والاستدامة المؤهلين تأهيلاً عالياً والمرخصين وذوي الخبرة الواسعة.",
      visionTitle: "رؤيتنا",
      ...
      services: [
        ...
        { title: "4. إدارة وتقييم المخاطر", items: ["تحليل مخاطر مكان عمل (JSA)", "تفتيش مواقع العمل", "تقييمات المخاطر"] },
        ...
        { title: "11. الحلول الرقمية للصحة والسلامة والبيئة", items: ["رقمنة نظام إدارة السلامة (SMS)", "لوحات مؤشرات رقمية", "أدوات رقمية للتدقيق"] },
        { title: "12. التحقيق في حوادث الصحة و السلامة المهنية", items: ["تحقيقات شاملة في الحوادث", "تحليل أنماط الحوادث", "رؤى لمنع التكرار"] }
      ]
    }
  };
  ...
  return (
    <main className={`min-h-screen bg-white font-sans ${lang === 'ar' ? 'text-right' : 'text-left'}`} dir={lang === 'ar' ? 'rtl' : 'ltr'}>
      ...
      {/* Navigation */}
      <nav className="p-4 border-b flex justify-between items-center bg-white sticky top-0 z-50 shadow-sm px-6 md:px-12">
      ...
      {/* Hero Section */}
      <header className="relative py-20 md:py-28 px-6 bg-gradient-to-b from-blue-50 to-white">
            ...
            <div className="flex flex-col gap-4 text-slate-800 font-bold">
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="flex flex-col">
                  <span className="text-slate-800 font-bold text-lg">+973 33430126</span>
                  <div className="flex gap-3 text-sm mt-1 font-semibold">
                    <a href="tel:+97333430126" className="text-blue-600 hover:text-blue-800 transition">Direct Call</a>
                    <span className="text-slate-300">|</span>
                    <a href={`https://wa.me/97333430126?text=${encodeURIComponent(current.whatsappMessage)}`} target="_blank" rel="noopener noreferrer" className="text-green-600 hover:text-green-800 transition">
                      WhatsApp
                    </a>
                  </div>
                </div>
          ...
          <div className="lg:col-span-3 bg-white p-8 md:p-10 rounded-[2rem] shadow-2xl shadow-blue-900/5 border border-slate-200">
            {status === 'success' ? (
              <div className="text-center py-8 animate-in zoom-in-95">
                ...
                <button onClick={() => setStatus('idle')} className="text-blue-900 font-bold border-b-2 border-blue-900 hover:text-blue-700 transition">
                  {current.backBtn}
                </button>
              </div>
            ) : (
              <form action={handleSubmit} className="space-y-6">
                <div>
                  <label className="block text-sm font-black text-black mb-2 uppercase">{current.nameLabel}</label>
                  <input name="name" required type="text" className="w-full p-4 rounded-xl border border-slate-300 bg-white text-black font-medium focus:ring-2 focus:ring-blue-900 outline-none" />
                </div>
                <div className="grid md:grid-cols-2 gap-6">
                  <div>
                    <label className="block text-sm font-black text-black mb-2 uppercase">{current.emailLabel}</label>
                    <input name="email" required type="email" className="w-full p-4 rounded-xl border border-slate-300 bg-white text-black font-medium focus:ring-2 focus:ring-blue-900 outline-none" />
                  </div>
                  <div>
                    <label className="block text-sm font-black text-black mb-2 uppercase">{current.phoneLabel}</label>
                    <input name="phone" required type="tel" className="w-full p-4 rounded-xl border border-slate-300 bg-white text-black font-medium focus:ring-2 focus:ring-blue-900 outline-none" />
                  </div>
                </div>
                <div>
                  <label className="block text-sm font-black text-black mb-2 uppercase">{current.messageLabel}</label>
                  <textarea name="message" required rows={4} className="w-full p-4 rounded-xl border border-slate-300 bg-white text-black font-medium focus:ring-2 focus:ring-blue-900 outline-none"></textarea>
                </div>
                {status === 'error' && <p className="text-red-600 font-bold mb-4">{current.errorMsg}</p>}
                <button disabled={isSubmitting} type="submit" className="w-full bg-blue-900 text-white font-black py-5 rounded-xl hover:bg-blue-800 transition disabled:bg-slate-400">
